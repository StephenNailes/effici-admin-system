...
export default function Sidebar() {
  ...
  // Force fetch notifications on mount if not already loaded
  useEffect(() => {
    if (notifications.length === 0 && unreadCount === 0) {
      console.log('🔔 [Sidebar] Forcing notification fetch on mount');
  ...
  const eventNotificationCount = (role === 'student' || role === 'student_officer')
    ? notifications.filter(n => !n.is_read && n.type === 'new_event').length 
    : 0;
    
  const announcementNotificationCount = (role === 'student' || role === 'student_officer')
    ? notifications.filter(n => !n.is_read && n.type === 'new_announcement').length
    : 0;
                  ...
                  <NotificationPanel 
                    ...
                    onOpen={() => {
                      ...
                      setTimeout(() => {
                        ...
                        // Don't fetch here - context handles it automatically
                      }, 200);
                    }}
                  />
                  {/* Show event/announcement notification indicators for students and student officers */}
                  {(role === 'student' || role === 'student_officer') && (eventNotificationCount > 0 || announcementNotificationCount > 0) && (
