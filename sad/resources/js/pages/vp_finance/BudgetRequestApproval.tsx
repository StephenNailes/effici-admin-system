...
export default function BudgetRequestApproval({ id }: Props) {
    ...
    axios
      ...
      .then((sigRes) => {
        if (sigRes?.data?.signatures) {
          ...
          // Show ALL existing signatures so VP Finance can see where previous approvers placed theirs
          setSignatures(list);
        }
      })
      .catch((err) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      router.visit('/vp-finance/requests');
    } catch (err) {
  ...
  const saveSignedDocument = async () => {
    ...
    try {
      ...
      if (pageRef.current) {
        const rect = pageRef.current.getBoundingClientRect();
        console.log('[VP Finance E-Sign] Saving NEW signatures with container info:', {
    ...
    <MainLayout>
      ...
      >
        ...
        >
          ...
          >
            <button
              onClick={() => router.visit('/vp-finance/requests')}
              className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
            >
              <ArrowLeft className="w-5 h-5 text-gray-600" />
            </button>
            <div className="flex-1">
              <h1 className="text-lg font-semibold text-gray-900 tracking-tight">Budget Request Approval (VP Finance)</h1>
        ...
        <div className="flex flex-1 pb-8">
          ...
          <div className="w-96 p-6">
            <div className="space-y-6">
              ...
              >
                <h3 className="text-lg font-semibold text-gray-900 mb-4">E-Signature (VP Finance)</h3>
                <p className="text-sm text-gray-600 mb-4">
                  Add your signature to the document before final approval.
                </p>
                {!isSignatureMode ? (
                  <button
