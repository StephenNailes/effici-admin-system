...
const FilterModal = ({
  ...
  statusFilter: string;
  onApply: (type: string, priority: string, status: string) => void;
}) => {
  ...
  const typeOptions = ["All Types", "Activity Plan", "Budget Request"];
  ...
  const handleApply = () => {
    onApply(tempTypeFilter, tempPriorityFilter, tempStatusFilter);
    onClose();
  };
...
export default function ActivityHistory() {
      ...
      .then((data) => {
        ...
        const mapped = (data.requests || [])
          ...
          return {
            ...
            type: item.request_type === 'budget_request' ? 'Budget Request' : 'Activity Plan',
  ...
  const handleApplyFilters = (type: string, priority: string, status: string) => {
    setTypeFilter(type);
  ...
  const filteredActivities = activities.filter((activity) => {
    ...
    return matchesType && matchesPriority && matchesStatus && matchesSearch && matchesDate;
  });
