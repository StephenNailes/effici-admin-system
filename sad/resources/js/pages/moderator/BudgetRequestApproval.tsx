...
export default function BudgetRequestApproval({ id }: Props) {
    ...
    axios
      ...
      .then((sigRes) => {
        if (sigRes?.data?.signatures) {
          ...
          // Show ALL existing signatures so moderator can see where previous approvers placed theirs
          setSignatures(list);
        }
      })
      .catch((err) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      router.visit('/moderator/requests');
    } catch (err) {
  ...
  const saveSignedDocument = async () => {
    ...
    try {
      ...
      if (pageRef.current) {
        const rect = pageRef.current.getBoundingClientRect();
        console.log('[Moderator E-Sign] Saving NEW signatures with container info:', {
    ...
    <MainLayout>
      ...
      >
        ...
        >
          ...
          >
            <button
              onClick={() => router.visit('/moderator/requests')}
              className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
            >
              <ArrowLeft className="w-5 h-5 text-gray-600" />
            </button>
            <div className="flex-1">
              <h1 className="text-lg font-semibold text-gray-900 tracking-tight">Budget Request Approval (Moderator)</h1>
        ...
        <div className="flex flex-1 pb-8">
          ...
          <div className="w-96 p-6">
            <div className="space-y-6">
              ...
              >
                <h3 className="text-lg font-semibold text-gray-900 mb-4">E-Signature (Moderator)</h3>
