...
import { Calendar, ArrowLeft, Save, AlertCircle, Link, Upload, X, ImageIcon } from 'lucide-react';
...
export default function CreateEvent() {
  ...
  const checkFormValidity = () => {
    const isValid = data.title.trim() !== '' && data.date !== '' && data.description.trim() !== '';
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validFiles.length > 0) {
      ...
      setData('images' as any, updatedImages);
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                {(errors as any).title ? (
                  ...
                  data.title.trim() && <p className="text-green-600 text-sm mt-1">✓ Title looks good</p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-red-400 transition-colors">
                  ...
                  <label htmlFor="images" className="cursor-pointer flex flex-col items-center gap-2">
                    ...
                    <div className="text-xs text-gray-500">PNG, JPG, GIF up to 10MB (Recommended: 1080x1350 for best display)</div>
                  </label>
                </div>
                ...
                {imagePreviews.length > 0 && (
                  <div className="mt-4">
                    <p className="text-sm font-medium text-gray-700 mb-2">Selected Images ({imagePreviews.length}/5)</p>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                      {imagePreviews.map((preview, index) => (
                        <div key={index} className="relative group">
                          ...
                          {index === 0 && (
                            <div className="absolute bottom-1 left-1 bg-red-600 text-white text-xs px-1 rounded">Primary</div>
                          )}
                        </div>
                      ))}
                    </div>
              ...
              {!isFormValid && (
                <p className="text-center text-xs text-gray-500 mt-2">Please fill in all required fields to create the event</p>
              )}
            </form>
          </div>
